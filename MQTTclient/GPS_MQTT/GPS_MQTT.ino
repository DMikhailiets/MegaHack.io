...
#define LED1 3
#define MQTT_client "Glados" // произвольное название MQTT клиента, иногда требуется уникальное.
...
const char *data_topic="test/led"; // топик для данных
...
void refreshData() {
  if (pause == 0) {
    ...
    distance_sm = digitalRead(LED1);
    Serial.println(distance_sm);
    client.publish(data_topic, String(distance_sm));
    pause = 1000; // пауза меду отправками 3 секунды
  }
  pause--;
